...
Building an end-to-end MLOps pipeline that spans local PyTorch development, multi-stage Docker containerization, and Kubernetes orchestration involved several non-trivial integration hurdles. The most challenging aspect was **coordinating state and storage across decoupled container lifecycles in Kubernetes while managing local cluster networking and image resolution**.
...
Configuring the Horizontal Pod Autoscaler (HPA) highlighted the dependency on cluster-level metrics infrastructure. In a minimal Minikube environment, without `metrics-server` enabled and configured with `--kubelet-insecure-tls`, the HPA reports `<unknown>` for CPU utilization targets. This demonstrated the practical distinction between defining declarative scaling policies in YAML and having the underlying telemetry available to act on them.

---

### Summary
This project reinforced the critical importance of declarative infrastructure, persistent volume lifecycle management, robust probe configurations, and clean separation of concerns between training and inference environments in modern MLOps.
